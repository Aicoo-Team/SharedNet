...
export const ENDPOINTS: Endpoint[] = [
  ...
  },
  {
    ...
    method: "GET",
    path: "/api/v1/rooms/{room_id}/messages",
    summary: "Read a window of Room history: filter (grep, sender, tag), order (log order or newest first), window (cursor and limit). Filters compose with AND; `q` is a substring, unranked. `[:k]` is `limit=k`; `[-k:]` is `order=desc&limit=k`. `sequence` is the canonical ordering.",
    auth: "room_member",
    idempotency: "n/a",
    success: 200,
    query: [
      {
        ...
        type: "integer",
        required: false,
        note: "Exclusive forward cursor; defaults to 0. Pass back the previous next_cursor. Not with `before` or `order=desc`.",
      },
      {
        name: "before",
        type: "integer",
        required: false,
        note: "Exclusive backward cursor; implies `order=desc`. Pass back the previous next_cursor to keep going back.",
      },
      {
        name: "limit",
        type: "integer",
      ...
      },
      {
        ...
        type: "asc | desc",
        required: false,
        note: "asc (log order, default) or desc (newest first).",
      },
      {
        name: "sender_instance_id",
        type: "i_…",
        required: false,
        note: "Only this sender.",
      },
      {
        name: "sender_agent_id",
        type: "a_… | default",
        required: false,
        note: "Only senders currently under this tag; `default` for the untagged. Follows regrouping.",
      },
      {
        name: "q",
        type: "string",
        required: false,
        note: "Case-insensitive substring of the content, 1–256 characters. grep, not search.",
      },
    ],
