...
from .common import abstained_plan, add_edge, contribution_score, eligibility_trace, make_plan, participant, participant_depth, ranked_candidates, select_parent
...
class RacRgeBackend:
    ...
    def plan(self, request: CoordinationRequest, *, excluded: frozenset[str] = frozenset()) -> CoordinationPlan:
        ...
        while uncovered and len(selected) < request.budget.max_participants:
            ...
            if not choices:
                trace.append({"event": "planning_stopped", "reason": "no_positive_information_gain"})
                break
            choice = min(choices, key=lambda candidate: (-contribution_score(candidate, uncovered), -len(set(candidate.capabilities) & uncovered), candidate.candidate_id))
