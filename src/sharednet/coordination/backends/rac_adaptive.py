...
from .common import abstained_plan, add_edge, eligibility_trace, empty_eligibility_stop_reason, make_plan, participant, participant_depth, ranked_candidates, select_parent
...
class RacAdaptiveBackend:
    ...
    def plan(self, request: CoordinationRequest, *, excluded: frozenset[str] = frozenset()) -> CoordinationPlan:
        ...
        stop_reason: str | None = "coverage_complete" if not uncovered else None
        if uncovered:
            for candidate in ranked_candidates(candidate for candidate in eligible if candidate.candidate_id != root.candidate_id):
                ...
                if candidate_utility(candidate, self.coordination_overhead) <= 0:
                    ...
                    continue
                if not fits_cost_budget(selected_cost, candidate.predicted_cost, request.budget.max_cost):
                ...
                selected_cost = sum_costs((selected_cost, candidate.predicted_cost))
