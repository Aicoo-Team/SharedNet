...
## 8. Deferred, with the constraints that make each non-trivial

- **Cross-Principal join by Room id** — wanted: knowing the id is the
  capability. Two blockers found: `ownedRoom` filters by the caller's Principal
  (a foreign Room is a 404), and `room_member` / `message` carry composite
  foreign keys to `room(principal_id, id)` that structurally forbid a member
  whose Principal differs from the Room's. Needs a migration, not an endpoint.
  Noted for the design: a Room id then becomes a bearer capability that appears
  in URLs, so its handling has to treat it as one.
...
- `0002` and `0003` are hand-ordered: drizzle-kit emits statements in an order
