...
| Unit and component | `pnpm test` (Vitest) | every PR, every push | protocol parsers, repositories (in-memory), the HTTP handler (including the credit purse: redemption once per account, payments by any id exactly once per key, refusals), Dashboard contracts and components, the CLI's verbs against the in-process handler, auth configuration |
