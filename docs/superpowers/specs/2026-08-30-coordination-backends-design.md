...
The prompt contains the bounded plan, a fresh nonce, exact participant roles, and an instruction for the root to spawn exactly `N-1` native subagents in parallel, wait for them, and synthesize every marker. Each recorded spawn prompt must decode to the exact task payload, participant assignment/capabilities, marker, and response contract for that child. Each child returns its marker followed by a nonempty contribution, and the corresponding final output must copy that contribution body exactly. No worker needs file or shell access for the smoke task.

The production adapter launches Codex from a fresh, empty temporary working directory so repository instructions and ambient files cannot silently alter the data-only proof. It parses stdout as JSONL, preserves stderr separately, records the thread ID, final message, collaboration events, and `turn.completed` usage. Acceptance requires exactly `N-1` successful `spawn_agent` events authored by the root, child IDs attributable to those exact payload-bearing spawns, completed root-authored waits covering exactly those children, and final non-root outputs equal to the marker-bound child contribution bodies. Spawn/wait are the only permitted collaboration controls for this data-only run; file, shell, web, or other disallowed tool activity fails closed. Timeout, nonzero exit, malformed or missing lifecycle evidence, missing markers, wrong participant count, wrong authorship, incomplete waits, unbound outputs, or disallowed tool use returns a typed failure. The service passes its original process-wide monotonic deadline to deadline-aware runtimes, so handoff and retries cannot restart the model-execution wall. On timeout the adapter sends `SIGTERM`, lets the process exit only for the request-deadline remainder (capped at ten seconds), and otherwise sends `SIGKILL` immediately. One bounded OS-cleanup allowance may run after the request deadline to collect output and close pipes; if that also times out, a daemon reaper holds the killed process until the OS reports its exit. Cleanup does not grant Codex more execution time.
