...
Resolve the binary from constructor, `SHAREDNET_CODEX_BINARY`, the ChatGPT app bundle, then `shutil.which("codex")`; run `--version` only in `availability()`, not before every task. `_default_runner` uses `subprocess.Popen`, `communicate(timeout=...)`, `terminate()`, and a termination grace capped by both ten seconds and the remaining request deadline before `kill()`. After `kill()`, one bounded OS-cleanup allowance collects output and closes pipes without extending model execution; a cleanup timeout hands the process to an eventual background reaper instead of adding a second foreground wait. Parse stdout line-by-line and ignore only non-JSON diagnostic lines. Preserve raw event count, collaboration event records, spawned agent thread IDs, root thread ID, final agent message, usage, exit code, and the last 4,000 stderr characters.

The prompt embeds JSON for the plan, exact markers, and the child response contract. For `N` participants, it tells the root to represent participant zero, issue exactly `N-1` parallel `spawn_agent` calls, copy the exact task and participant contract into each spawn prompt, wait for every child, and return a JSON object containing all markers and contributions. Production launches in a fresh empty temporary working directory. Acceptance requires exactly `N-1` successful root-authored exact-payload spawn events, completed root-authored waits covering those same children, marker-first child contributions, and exact equality between each child body and its final non-root output. It forbids file/shell/web tools for the data-only smoke, and any such disallowed tool evidence fails closed. The service uses an optional deadline-aware runtime capability to propagate one absolute monotonic deadline without breaking legacy `execute(plan)` implementations; Codex clamps that deadline against the plan-local budget.
